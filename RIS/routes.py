...
import secrets, os
from flask import render_template, url_for, flash, redirect, request, abort, json, send_file
    ...
    if current_user.is_authenticated:
        if current_user.email[-5] == 'A':
            ...
            if form.validate_on_submit():
                ...
                doctor = Doctor(id=password, name=form.name.data, ssn=form.ssn.data, email=form.email.data, dob=form.dob.data, speciality=form.speciality.data, salary=form.salary.data, gender=0, age=age)
        ...
        if current_user.email[-5] == 'A':
            ...
            if form.validate_on_submit():
                ...
                receptionist = Receptionist(id=password, name=form.name.data, ssn=form.ssn.data, email=form.email.data,
                                    dob=form.dob.data, salary=form.salary.data, gender=form.gender.data, address=form.address.data, age=age)
                user = User(id=password, username=password, email=mail, password=hashed_password)
...
def save_csv():
    ...
    scans = Scan.query.all()
    with open('Radiology_Center_Data1.csv', 'w', encoding='utf-8', newline='' ) as csvfile:
            ...
            rows.append(
                {
                    ...
                    'Scan_Fees': scan.fees,
                    'Receptionist_Name': scan.receptionist.name,
                    'Receptionist_Salary': scan.receptionist.salary,
                    'Receptionist_Gender': scan.receptionist.gender,
                    'Receptionist_Age': scan.receptionist.age,
                    'Technician_Name': scan.technician.name,
                    'Technician_Salary': scan.technician.salary,
                    'Technician_Gender': scan.technician.gender,
                    'Technician_Age': scan.technician.age,
                    'Doctor_Name': scan.doctor.name,
                    'Doctor_Speciality': scan.doctor.speciality,
                    'Doctor_Salary': scan.doctor.salary,
                    'Doctor_Gender': scan.doctor.gender,
                    'Doctor_Age': scan.doctor.age
